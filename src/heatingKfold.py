...
for fold, (train_idx, val_idx) in enumerate(kf.split(x_train, y_train), 1):
    ...
    rmse_f2 = root_mean_squared_error(y_val, y_val_pred)
...
while i < 11:
    ...
    heating = model.predict([x_scaled], verbose=0).flatten()
    heating2 = model.predict([x2_scaled], verbose=0).flatten()
    heating = float(heating.item())
    heating2 = float(heating2.item())
    print(f'heating({i}): {heating}')
    c = heating - heating2
    print(f'Hminus: {c}')
    print('--------------------------')
    i += 1
...
axs[0].set_title('Heating Load (train data)')
axs[0].set_xlabel('Actual Value (MWh)')
axs[0].set_ylabel('Predicted Value (MWh)')
axs[1].set_title('Heating Load (test data)')
