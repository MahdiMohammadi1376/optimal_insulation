...
The tool supports four insulation materials (Rock Wool, XPS, EPS, Glass Wool) and can optimize for a single material or all four simultaneously, producing a **Pareto front** that represents the trade-off between cost and emissions.
...
2. A **Coefficient Converter model** scales these predictions to match the user's actual building dimensions (gross floor area, external wall area, WWR), enabling generalization beyond the training geometry.
...
│
├── models/                  # Trained Keras models & scalers (not tracked by git)
│   ├── heatingKfold2.h5
│   ├── coolingKfold2.h5
│   ├── coefKfold9.h5
│   ├── scaler_thermal.pkl
...
```bash
git clone https://github.com/your-username/insulation-optimizer.git
...
Place your trained `.h5` and `.pkl` files into the `models/` directory. The expected filenames are:

| File | Description |
|------|-------------|
| `heatingKfold2.h5` | Heating load ANN model |
| `coolingKfold2.h5` | Cooling load ANN model |
| `coefKfold9.h5` | Coefficient converter ANN model |
| `scaler_thermal.pkl` | Feature scaler for thermal models |
| `scaler_convertor.pkl` | Feature scaler for converter model |
...
```
Enter geometry information in order 'A1', 'A2', 'A6': 500, 300, 0.30
Enter material information in order 'R_wall', 'U_glass', 'SHGC', 'COP cooling', 'COP heating': 0.5, 1.8, 0.4, 3.0, 2.5
Enter prices in order 'Insulation per square meter', 'Gas', 'Electricity': 450000, 0.05, 0.12
Cooling fuel consumption (Gas or Electricity): electricity
Heating fuel consumption (Gas or Electricity): gas
Useful life of the building (years): 50
...
The **coefficient converter** model maps loads from the reference simulation geometry to user-specified building dimensions.
...
```bibtex
@software{insulation_optimizer,
  author    = {Your Name},
  title     = {Building Insulation Optimizer: Multi-Objective Optimization with ANN and NSGA-II},
  year      = {2025},
  url       = {https://github.com/your-username/insulation-optimizer}
}
```
...
Contributions are welcome! Please open an issue or submit a pull request for bug fixes, new features, or additional insulation materials.
